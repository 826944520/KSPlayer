 ...
 *
 * FFmpeg 9.0 changed the first parameter from `AVFormatContext *` to
 * `void *` (generic logging context) and const-qualified `AVCodecParameters`.
 * The Swift call site passes no AVFormatContext, so this shim is the only
 * place the signature matters.
 *
 * @param logctx address of the logging context.
